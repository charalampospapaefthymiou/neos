...
export default async function handler(req) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { slug, treatment_id, date, time, name, phone, email, opt_in } = await req.json();
      if (!slug || !treatment_id || !date || !time || !name || !phone || !email) return json({ error: 'Bitte alle Felder ausfüllen' }, 400);
      ...
      if (!customer) {
        const ins = await rest('customers', {
          ...
          body: JSON.stringify({ salon_id: salon.id, name: name.slice(0, 120), phone: phone.slice(0, 40), email: email.slice(0, 200), ...optInFields }),
        });
        customer = ins[0];
      } else {
        // Bestehende Kundin: E-Mail ergänzen (falls noch leer) + Einwilligung dokumentieren/erneuern
        const patch = { ...optInFields };
        if (!customer.email) patch.email = email.slice(0, 200);
        if (Object.keys(patch).length) await rest(`customers?id=eq.${customer.id}`, { method: 'PATCH', body: JSON.stringify(patch) });
      }
